...
namespace Library_Management_System.Web.Models
{
    public class JwtMiddleware
    {
        ...
        public JwtMiddleware(RequestDelegate next, IOptions<AppSettings> appSettings/* ApplicationDBContext context*/)
        {
            _next = next;
        ...
        private async Task attachAccountToContext(HttpContext context, ApplicationDBContext dataContext, string token)
        {
            try
            {
                ...
                context.Items["ApplicationUser"] = await dataContext.Users.FindAsync(accountId);
            }
            catch
            {
